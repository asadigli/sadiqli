...
PR_NOCODE:"nocode",PR_PLAIN:"pln",PR_PUNCTUATION:"pun",PR_SOURCE:"src",PR_STRING:"str",PR_TAG:"tag",PR_TYPE:"typ"}})();
		...
		jQuery(document).ready(function($) {
		  ...
		  $('.cbp-l-loadMore-button-link').on('click', function(e) {
			...
			window.onscroll = function() {HScrollFunc()};
			var header = document.getElementById("myHeader");
			var sticky = header.offsetTop;
			function HScrollFunc(){
			...
			jQuery(document).ready(function(){
				...
				jQuery('#AddSubscription').click(function(e){
					e.preventDefault();
					$.ajaxSetup({ headers: {'X-CSRF-TOKEN': $('meta[name="_token"]').attr('content')}});
					jQuery.ajax({
						url: "/addnewsubscription",
						method: 'POST',
						data: { email: jQuery('#SubscribeEmail').val(), },
							error: function(result){
								$( ".hidden_alert" ).empty().css('display','none');
								jQuery('#SubscribeEmail').css('border','1px solid red');
								$( "#subs_sent_failed" ).append( "<b>Zəhmət olmasa, xananı doldurun!</b>" ).css('display','').delay(14000).fadeOut();
						},
						success: function(result){
								$( ".hidden_alert" ).empty().css('display','none');
								jQuery('#SubscribeEmail').css('border','1px solid #ccc').val("");
								$( "#subs_sent" ).append( "Uğurla həyata keçdi. Diqqətiniz üçün təşəkkür edirik!" ).css('display','').delay(14000).fadeOut();
					}});
				});
				jQuery('#AddComment').click(function(e){
					...
					jQuery.ajax({
						url: "/addnewcomment",
						method: 'POST',
						data: { name: jQuery('#comment_name').val(), email: jQuery('#comment_email').val(), comment: jQuery('#comment_body').val(), blog_id: jQuery('#comment_blog_id').val(), },
							error: function(result){
								$( "#comment_sent_failed" ).css('display','none');
								$( "#comment_sent" ).css('display','none');
								if (!$('#commenter_name').val()){
									jQuery('#comment_name').css('border','1px solid red');
								}if (!$('#comment_email').val()){
									jQuery('#comment_email').css('border','1px solid red');
								}if (!$('#comment_body').val()){
									jQuery('#comment_body').css('border','1px solid red');
								}
								$( "#comment_sent_failed" ).css('display','').delay(30000).fadeOut();
						},
						success: function(result){
								$( "#comment_sent_failed" ).css('display','none');
								$( "#comment_sent" ).css('display','none');
								jQuery('#comment_name').css('border','1px solid #ccc').val("");
								$("#divToReload").load(location.href+" #divToReload>*","");
								jQuery('#comment_email').css('border','1px solid #ccc').val("");
								jQuery('#comment_body').css('border','1px solid #ccc').val("");
								$( "#comment_sent" ).css('display','').delay(30000).fadeOut();
					}});
				});
				jQuery('#LikeBlog').click(function(e){
					e.preventDefault();
					$.ajaxSetup({ headers: {'X-CSRF-TOKEN': $('meta[name="_token"]').attr('content')}});
					jQuery.ajax({
						url: "/likeblog",
						method: 'POST',
						data: { eblog_liker: jQuery('#blog_like_email').val(), blog_liker: jQuery('#blog_like_name').val(), bl_id: jQuery('#like_blog_id').val(), },
							error: function(result){
									$( "#blog_like_failed" ).css('display','none');
									$( "#blog_liked" ).css('display','none');
								if (!$('#blog_like_name').val()){
									jQuery('#blog_like_name').css('border','1px solid red');
								}if (!$('#blog_like_email').val()){
									jQuery('#blog_like_email').css('border','1px solid red');
								}
								$( "#blog_like_failed" ).css('display','').delay(30000).fadeOut();
						},
						success: function(result){
								$( "#blog_like_failed" ).css('display','none');
								$( "#blog_liked" ).css('display','none');
								$("#LikeSectionUpdate").load(location.href+" #LikeSectionUpdate>*","");
								$( ".like_div" ).empty().css('display','none');
								jQuery('#blog_like_name').css('border','1px solid #ccc').val("");
								jQuery('#blog_like_email').css('border','1px solid #ccc').val("");
								$( "#blog_liked" ).css('display','');
					}});
				});
		      ...
		      jQuery.ajax({
		        ...
		        data: { name: jQuery('#name').val(), email: jQuery('#email').val(), title: jQuery('#title').val(), message: jQuery('#message').val(), },
		          error: function(result){
		            ...
		            $( "#sent_failed" ).css('display','').delay(8000).fadeOut();
		        },
		        success: function(result){
		            $( "#sent" ).css('display','none');
		            $( "#sent_failed" ).css('display','none');
		            ...
		            $( "#sent" ).css('display','').delay(13000).fadeOut();
		      }});
		    });
		  });
